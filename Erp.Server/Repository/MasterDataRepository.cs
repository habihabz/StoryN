...
namespace Erp.Server.Repository
{
    public class MasterDataRepository : IMasterData
    {
        ...
        public MasterData getCountry(RequestParams requestParams)
        {
            var _name = new SqlParameter("name", requestParams.name + "");
            var masterdata = db.Set<MasterData>().FromSqlRaw("EXEC dbo.getCountry @name;", _name).ToList().FirstOrDefault() ?? new MasterData();
